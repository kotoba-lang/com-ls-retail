(ns ls_retail.main
  "Kotodama WASM entrypoint for the Ls_retail clean-room actor (L4) — Clojure port.
  ...
  facts (`ls_retail.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ls_retail")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "lsretail_pro"
   ...
   {:entity "Customer"  :plural "customers"  :id-prefix "lsretail_cus"
    :fields [:email :firstName :lastName :phone] :required [:email :firstName]
    :coerce {}                               :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "lsretail_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"ls_retail.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ls_retail-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ls_retail-compat/ready)
